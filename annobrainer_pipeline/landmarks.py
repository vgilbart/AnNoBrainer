...
from annobrainer_pipeline.external.landmarks.NBB.algorithms import neural_best_buddies as NBBs
from annobrainer_pipeline.external.landmarks.NBB.options.options import Options
from annobrainer_pipeline.external.landmarks.NBB.util import util
from annobrainer_pipeline.external.landmarks.warp import umeyama

import ssl  # NOQA

ssl._create_default_https_context = ssl._create_unverified_context  # NOQA
from annobrainer_pipeline.external.landmarks.NBB.models import vgg19_model  # NOQA


# FUNCTION TO FIND MATCHING LANDMARKS ==========================================
def main(
    content_path="moving", style_path="fixed", gpu_ids=0, path_results=""
):
    # NBB
    ...
    # NBB arguments
    im_size = 224
    pts_path = str(path_results / "NBBresults")
    # Clean points argumetns
    content_pts_path = str(path_results / "NBBresults/correspondence_A.txt")
    style_pts_path = str(path_results / "NBBresults/correspondence_B.txt")
    activation_path = str(
        path_results / "NBBresults/correspondence_activation.txt"
    )
    output_path = str(path_results / "CleanedPts")
    ...
    nbbs.run(A, B, opt.datarootA, opt.datarootB)
    ...
    if NBB:
        if True:
            ...
            for i in range(A_corres.shape[0]):
                plt.plot(
                    A_corres[i][1], A_corres[i][0], marker="o", markersize=10
                )
            ...
            plt.savefig(
                output_path + "/A_all_correspondence.png",
                bbox_inches="tight",
                pad_inches=0,
            )
            plt.close()
        ...
        if True:
            ...
            for i in range(B_corres.shape[0]):
                plt.plot(
                    B_corres[i][1], B_corres[i][0], marker="o", markersize=10
                )
            plt.gca().set_axis_off()
            plt.subplots_adjust(
                top=1, bottom=0, right=1, left=0, hspace=0, wspace=0
            ...
            plt.gca().yaxis.set_major_locator(plt.NullLocator())
            plt.savefig(
                output_path + "/B_all_correspondence.png",
                bbox_inches="tight",
                pad_inches=-0.1,
            )
            plt.close()
        ...
        print(
            "\n{} points removed because their activations were less than 1 (NBB-specific processing)".format(
                lowactv[0].shape[0]
            )
        )
        ...
        for i in range(max_num_points):
            ...
            rcondition = np.logical_and(
                A_corres[:, 0] >= rleft, A_corres[:, 0] <= rright
            )
            ccondition = np.logical_and(
                A_corres[:, 1] >= cleft, A_corres[:, 1] <= cright
            )
            condition = np.logical_and(rcondition, ccondition)
            activations[condition] *= 0.0
        ...
        for i in range(max_num_points):
            ...
            B_curr = B_selected[i]
            if np.logical_and(
                np.all(A_curr == A_prev), np.all(B_curr == B_prev)
            ):
                repeats.append(i)
            A_prev = A_selected[i]
            B_prev = B_selected[i]
        ...
        print(
            "\n{} points selected in a greedy way (NBB-specific processing)".format(
                A_selected.shape[0]
            )
        )
    ...
    def get_target_pts(A_selected, B_selected):
        ...
        T = umeyama(
            src=src_pts.data.cpu().numpy(),
            dst=target_pts.data.cpu().numpy(),
            estimate_scale=True,
        )
        T = torch.from_numpy(T).float()

        target_pts_padded = torch.cat(
            (target_pts, torch.ones((target_pts.size(0), 1))), 1
        )
        new_target_pts = torch.matmul(
            torch.inverse(T), torch.transpose(target_pts_padded, 0, 1)
        )
        new_target_pts = torch.transpose(new_target_pts[:2], 0, 1)
        new_target_pts[:, 0] = torch.clamp(
            new_target_pts[:, 0], min=1, max=A_im.shape[0] - 1
        )
        new_target_pts[:, 1] = torch.clamp(
            new_target_pts[:, 1], min=1, max=A_im.shape[1] - 1
        )
        ...
        for i in range(A_selected.shape[0]):
            ...
            for j in range(i + 1, A_selected.shape[0]):
                ...
                if R:
                    ...
                    if (
                        R[0] >= max(i_xmin, j_xmin)
                        and R[0] <= min(i_xmax, j_xmax)
                        and R[1] >= max(i_ymin, j_ymin)
                        and R[1] <= min(i_ymax, j_ymax)
                    ):
                        inds.append(j)
    ...
    A_selected, B_selected, B_selected_new, act_selected = remove_crossing(
        A_selected, B_selected, B_selected_new, act_selected
    )
    ...
    print(
        "\n{} points remaining after removing crossing points".format(
            A_selected.shape[0]
        )
    )
    ...
    if True:
        ...
        for i in range(A_selected.shape[0]):
            ...
            plt.arrow(
                A_selected[i][1],
                A_selected[i][0],
                B_selected_new[i][1] - A_selected[i][1],
                B_selected_new[i][0] - A_selected[i][0],
                color=colors[i],
                width=0.5,
            )
        plt.gca().set_axis_off()
        ...
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig(
            output_path + "/A_source_and_target_final.png",
            bbox_inches="tight",
            pad_inches=0,
        )
        plt.close()
    ...
    if True:
        ...
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig(
            output_path + "/A_selected_final.png",
            bbox_inches="tight",
            pad_inches=0,
        )
        plt.close()
    ...
    if True:
        ...
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig(
            output_path + "/B_selected_final.png",
            bbox_inches="tight",
            pad_inches=0,
        )
        plt.close()
    ...
    with open(
        output_path + "/correspondence_activation.txt", "wt"
    ) as opt_file:
        for j in range(act_selected.shape[0]):
            opt_file.write("%f\n" % (act_selected[j]))

    print(
        "\nSaved cleaned content points at",
        output_path + "/correspondence_A.txt",
    )
    print(
        "Saved cleaned style points at", output_path + "/correspondence_B.txt"
    )
    print(
        "Saved cleaned activations at",
        output_path + "/correspondence_activation.txt",
    )
